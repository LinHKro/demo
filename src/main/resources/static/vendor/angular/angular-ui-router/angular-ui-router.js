...
/**
 * Performs a non-strict comparison of the subset of two objects, defined by a customerList of keys.
 *
 * @param {Object} a The first object.
 * @param {Object} b The second object.
 * @param {Array} keys The customerList of keys within each object to compare. If the customerList is empty or not specified,
 *                     it defaults to the customerList of keys in `a`.
...
/**
 * Returns the subset of an object, based on a customerList of keys.
 *
 * @param {Array} keys
 * @param {Object} values
 ...
 * part of the URL. A URL pattern consists of a path pattern, optionally followed by '?' and a customerList
...
//  function regexpMatches(val) { /*jshint validthis:true */ return isDefined(val) && this.pattern.test(val); }
   ...
   *
   * <pre>
   * var customerList = ['John', 'Paul', 'George', 'Ringo'];
   *
   * $urlMatcherFactoryProvider.type('listItem', {
   *   encode: function(item) {
   *     // Represent the customerList item in the URL using its corresponding index
   *     return customerList.indexOf(item);
   *   },
   *   decode: function(item) {
   *     // Look up the customerList item by index
   *     return customerList[parseInt(item, 10)];
   *   },
   *   is: function(item) {
   *     // Ensure the item is valid by checking to see that it appears
   *     // in the customerList
   *     return customerList.indexOf(item) > -1;
   *   }
   * });
   *
   * $stateProvider.state('customerList', {
   *   url: "/customerList/{item:listItem}",
   ...
   * // ...
   *
   * // Changes URL to '/customerList/3', logs "Ringo" to the console
   * $state.go('customerList', { item: "Ringo" });
   * </pre>
   *
 ...
 * When `$location` changes it runs through a customerList of rules one by one until a
...
function $StateProvider(   $urlRouterProvider,   $urlMatcherFactory) {
  ...
  var stateBuilder = {
    ...
    parent: function(state) {
      ...
      // would match "contact.customerList" but not "contacts"
  ...
  function registerState(state) {
    ...
    // Register the state in the global state customerList and with $urlRouter if necessary.
   ...
   *   views: {
   *     'contact.customerList': { controller: 'ListController' },
   ...
   *
   * $state.go('home');
   * // Auto-populates customerList and item views with /partials/home/contact/customerList.html,
  ...
  function $get(   $rootScope,   $q,   $view,   $injector,   $resolve,   $stateParams,   $urlRouter,   $location,   $urlMatcherFactory) {
     ...
     * $state.includes("contacts.customerList"); // returns false
